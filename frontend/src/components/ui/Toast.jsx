...
const Toast = ({ message, type = 'success', onClose, duration = 4000 }) => {
    ...
    return (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-[100] animate-fade-in">
            <div className={`flex items-center gap-3 px-5 py-4 rounded-2xl shadow-2xl border backdrop-blur-md bg-white/90 dark:bg-gray-800/90 min-w-[280px] max-w-md ${getStyles()}`}>
                {getIcon()}
                <p className="font-medium text-sm flex-1">{message}</p>
                <button onClick={onClose} className="flex-shrink-0 p-1 rounded-full hover:bg-black/5 dark:hover:bg-white/10 transition-colors">
                    <X size={16} />
                </button>
            </div>
        </div>
